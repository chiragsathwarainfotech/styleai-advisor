...
const Auth = () => {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      // iOS needs its own OAuth client id; Android/web continue using the web client id.
      const googleClientId =
        Capacitor.getPlatform() === "ios"
          ? "831294607412-65fgl8bvvg3nrea5k27ji2lg77nut7ku.apps.googleusercontent.com"
          : "831294607412-3sid3g29q4jj4p1jbr8qke8aabhmh050.apps.googleusercontent.com";
  ...
  const handleAppleSignIn = async () => {
    ...
    try {
      // Apple + Supabase nonce handshake: Apple receives the SHA-256 hash of a
      // random nonce (stored in the id_token), Supabase receives the raw nonce
      // and re-hashes it to verify. Both must be present or Supabase rejects it.
      const rawNonce = Array.from(crypto.getRandomValues(new Uint8Array(16)))
        .map((b) => b.toString(16).padStart(2, "0"))
        .join("");
      const hashedNonce = Array.from(
        new Uint8Array(await crypto.subtle.digest("SHA-256", new TextEncoder().encode(rawNonce)))
      )
        .map((b) => b.toString(16).padStart(2, "0"))
        .join("");
                ...
                <>
                  ...
                  {Capacitor.getPlatform() === "ios" && (
                    <Button
                      type="button"
                      variant="outline"
                      className="w-full h-12 mt-6 font-body font-medium border-border/50 hover:bg-muted/50 gap-3 bg-black text-white hover:bg-black/90 border-black"
